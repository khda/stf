...
dbapi.createBootStrap = function(env) {
  ...
  return dbapi.createGroup({
    ...
    .then(function(group) {
      ...
      , ip: '127.0.0.1'
      , password: env.STF_ADMIN_PASSWORD
      })
      .then(function() {
...
dbapi.saveUserAfterLogin = function(user) {
  return db.run(r.table('users').get(user.email).update({
    ...
    .then(function(stats) {
      if (stats.skipped) {
        return dbapi.createUser(user.email, user.name, user.ip, user.password)
      }
      return stats
    })
}
